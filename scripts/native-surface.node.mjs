...
test("the native surface section names its proxy nodes and deliver verbs", () => {
  ...
  assert.match(spec, /`snapshot`, `state`, `read`, `scroll`, `wheel`, `selection`, `focus`, `input`, `theme`, `stop` and `archive`/);
...
test("the Korean contract carries the same native surface rules", () => {
  ...
  assert.match(spec, /`snapshot`, `state`, `read`, `scroll`, `wheel`, `selection`, `focus`, `input`, `theme`, `stop`, `archive`/);
